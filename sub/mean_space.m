function [lat,mat,vs] = mean_space(datmat, datlat, vs, crop)
...
%   matrix.
%
% FORMAT [lat,mat,vs] = mean_space(datmat, datlat, vs, crop)
...
crop = pad(crop(:), [3-numel(crop) 0], 'replicate', 'post');
vs   = pad(vs(:),   [3-numel(vs)   0], 'replicate', 'post');

% Remove spurious mat/lat
%--------------------------------------------------------------------------
datmat = reshape(datmat, 4, 4, []);
datlat = reshape(datlat, 3, []);
if size(datmat, 3) ~= size(datlat, 2)
    error('There should be as many matrices as lattices.');
end
msk = reshape(any(any(~isfinite(datmat))), [], 1) | ...
      reshape(any(~isfinite(datlat)), [], 1);
datmat(:,:,msk) = [];
datlat(:,msk)   = [];
...
%--------------------------------------------------------------------------
is2d = datlat(3,1) == 1;
if is2d, B = affine_basis('SE(2)');
else,    B = affine_basis('SE(3)'); end
